...
| Discovery | `mysql-exporter-scrape-path` | A fresh 25 September run captured external Prometheus Targets screenshots before, during and after the fault: the discovered exporter changed from UP at `/metrics` to DOWN at `/metrics-v2` with HTTP 404, then recovered to UP. FCAPSule's ready assessment incorrectly treated an unrelated dropped target in another ServiceMonitor as the cause, and a later co-firing alert displaced the run's exact primary incident. This is not a diagnosis pass. | Prioritize evidence from the alert's target and scrape pool, distinguish a discovered 404 from an unrelated dropped target, and preserve the selected primary incident through correlated alerts. Re-run only after those general Product fixes. |
