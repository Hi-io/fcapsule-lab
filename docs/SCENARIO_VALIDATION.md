...
| Logs | `transaction-deadlock` | A fresh 25 September run produced the exact deadlock alert and a new episode, then recovered. Logs show real MySQL 1213 victims and opposite SKU orders. FCAPSule's unpublished draft identified lock-order inversion, but its optional historical comparison had no citation; validation therefore published an inconclusive assessment. A second same-name alert also became the episode's primary incident. This is not a qualified demo. | Omit unsupported optional historical claims without discarding separately grounded current findings, and keep the selected incident stable across same-alert co-firings. Reassess the retained capsule once after a general Product fix; do not repeat the database fault just to improve a score. |
...
| Discovery | `mysql-exporter-scrape-path` | Two fresh 25 September runs captured real Prometheus Targets screenshots: the discovered exporter changed from UP at `/metrics` to DOWN at `/metrics-v2` with HTTP 404, then recovered to UP. The first investigation misdiagnosed a dropped target in another pool and lost its exact primary incident. After a general Product target-scope fix, the second isolated run reached a ready, incident-pinned assessment that correctly named `/metrics-v2`, HTTP 404, a Ready pod, and a path/configuration check. Its external screenshot was delivered to the vision model and cited in a ready reassessment. However, that reassessment also cited an unrelated ServiceMonitor's dropped target as supporting evidence; the optional consistency review was unavailable. This remains a partially helpful, unqualified demo, not a clean image-assisted diagnosis. | Keep non-target monitor selections out of the compact reasoning context when an exact active DOWN target exists, while retaining them in the full capsule. Verify image-assisted synthesis against that scoped context and preserve the correct incident through co-firing alerts before promotion. Do not repeat the fault simply to improve a score. |
